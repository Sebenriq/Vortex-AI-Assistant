...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = $pdo->prepare(
            'INSERT INTO pacientes (nombre, edad, genero, alergias, antecedentes, descriptor_facial)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([
            $datos['nombre'],
...
<div class="d-flex align-items-center mb-4 gap-2">
  ...
  <h4 class="fw-bold mb-0">Registrar paciente</h4>
</div>
  ...
  <div class="card-body">
    <form method="POST" novalidate>
      ...
      <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save me-1"></i>Guardar paciente</button>
